...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/caddy_v1.0.3_linux_amd64.tar.gz"
	elif [[ ${bit} == "i386" || ${bit} == "i686" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/caddy_v1.0.3_linux_386.tar.gz"
	elif [[ ${bit} == "armv7l" ]]; then
		wget --no-check-certificate -O "caddy_linux.tar.gz" "https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/caddy_v1.0.3_linux_arm7.tar.gz"
	else
	...
	rm -rf "init/"
	chmod +x caddy
	wget --no-check-certificate -O "www.tar.gz" "https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/www.tar.gz"
	tar zxf "www.tar.gz"
	rm -rf www.tar.gz
	wget --no-check-certificate -O "Caddyfile" "https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/Caddyfile"
}
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/caonimagfw/Caddy/master/download/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
...
install_caddy(){
 ...
 /etc/shadowsocks.json："redirect": ["*:80#127.0.0.1:8099"],
 http端口：8099
